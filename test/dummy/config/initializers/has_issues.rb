...
HasIssues.configure do |config|
  # Engine Configuration

  # current_user_proc
  # Type: Proc
  # Arguments: the current controller (ActionController::Base)
  # Returns: the current user (acts_as_issuer)
  ...
  # Returns: a String that is appended to HasIssues JS views
  ...
  # User (acts_as_issuer) Configuration

  # user_name_proc
  # Type: Proc
  # Arguments: a user (acts_as_issuer)
  # Returns: the user's name (String)
  # Default: lambda { |user| I18n.t('has_issues.anonymous') } (all users are anonymous)
  config.user_name_proc = lambda { |user| I18n.t('has_issues.anonymous') }
  ...
  # user_avatar_proc
  # Type: Proc
  # Arguments: a user (acts_as_issuer), a view (ActionView::Base)
  # Returns: a String containing a HTML <img> tag pointing to the user's avatar image
  # The issuer_gravatar_image_tag helper takes a user object,
  ...
  #            view.issuer_gravatar_image_tag(
  #              user, 1, :s => 60, :d => 'mm') }
  ...
  # user_email_proc
  # Type: Proc
  # Arguments: a user (acts_as_issuer), a mailer (ActionMailer::Base)
  # Returns: the user's email address (String)
  # The default works for Devise's defaults
  # If the mailer argument is nil, HasIssues intends to hash the email and send the hash
  # to Gravatar, so you should always return the user's email address (if using Gravatar)
  # If the mailer argument is not nil, then HasIssues intends to send an email to
  ...
  # Issue/Issueable (has_issues) Configuration
  ...
  # issue_read_proc
  # Type: Proc
  # Arguments: a issue (HasIssues::Issue), a user (acts_as_issuer)
  # Returns: a Boolean, true if and only if the user should be allowed to read that issue
  # Note: can be called with a user object that is nil (if they are not logged in)
  # Default: lambda { |issue, user| true } (anyone can read any issue)
  config.issue_read_proc = lambda { |issue, user| true }

  # issue_moderator_proc
  # Type: Proc
  # Arguments: a issue (HasIssues::Issue), a user (acts_as_issuer)
  # Returns: a Boolean, true if and only if the user is a moderator for that issue
  # If you want global moderators, make this proc true for them regardless of issue
  # Default: lambda { |issue, user| false } (no moderators)
  config.issue_moderator_proc = lambda { |issue, user| false }
  ...
  # If :l is selected, the "reply to issue" form will appear before the comments
  ...
  # issue_subscription
  # Type: Symbol
  # Whether users can subscribe to issues to receive activity email notifications
  # Valid options:
  #   :n (no subscriptions)
  ...
  # Default: :n
  config.issue_subscription = :n

  # email_from_proc
  # Type: Proc
  # Arguments: an issue (HasIssues::Issue)
  ...
  # Type: Proc
  # Arguments: an issue (HasIssues::Issue)
  # Returns: a name that refers to the issueable object (String)
  # If you have multiple issueable models, you can also pass this
  # configuration value as an argument to has_issues for each one
  # Default: lambda { |issue|
  #                   "#{issue.issueable.class.name} ##{issue.issueable.id}" }
  config.issueable_name_proc = lambda { |issue|
    "#{issue.issueable.class.name} ##{issue.issueable.id}" }

  # comment_url_proc
  # Type: Proc
  # Arguments: a comment (HasIssues::Comment),
  #            the app_routes (ActionDispatch::Routing::RoutesProxy)
  # Returns: a String containing the url of the view that displays the given comment
  # This usually is the issueable's "show" page
  # The main application's routes can be accessed through the app_routes object
  # Default: lambda { |comment, app_routes|
  #                   app_routes.polymorphic_url(comment.issue.issueable,
  #                                              anchor: "comment_#{comment.id}_div") }
  # (defaults to the issueable's show url with an anchor pointing to the comment's div)
  config.comment_url_proc = lambda { |comment, app_routes|
    app_routes.polymorphic_url(comment.issue.issueable, anchor: "comment_#{comment.id}_div") }
